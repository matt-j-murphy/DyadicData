...
#centering on your own

#create person mean
conflict_pm <- aggregate(conflict ~ id, data = process, FUN = mean)
names(conflict_pm) <- c("id", "conflict_pm")

#create grand mean
conflict_gm <- mean(process$conflict)
#or mean(conflict_pm[,2])
#here, it does not make a difference, as there are no missing values
#otherwise you need to decide what is more suitable! 

#attach grand mean and person mean to your dara frame
process$conflict_gm <- conflict_gm 
process <- merge(process, conflict_pm, by = "id", all = F)

#grand mean centered score (for between-person effects; equal to confcb)
#Has a person in general more or less conflict than the average parson?
process$clonflict_gmc <- process$conflict_pm - process$conflict_gm

#person mean centered score (for within-person effects; equal to confcw)
#On a certain day, has a person more or less conflict than usual?
process$clonflict_pmc <- process$conflict - process$conflict_pm 

#transform time variable to indicate 0 = First day and 1 = Last day
#is more useful than time7c!
process$time01 <- process$time/27 
...
cpmodel0 <- lme(fixed=intimacy ~ 1, data = process, random = ~ 1 | id)
summary(cpmodel0)

#linear growth model with AR(1) errors 
...
# linearity? Plot each predictor against residuals

plot(resid(cpmodel) ~ process$confcw,
     xlab = "Confcw", ylab = "Residuals")

plot(resid(cpmodel) ~ process$relqual,
     xlab = "Relqual", ylab = "Residuals")

plot(resid(cpmodel) ~ process$confcb,
     xlab = "Confbw", ylab = "Residuals")

plot(resid(cpmodel) ~ process$time7c,
     xlab = "intimacy (observed)", ylab = "Residuals")

# heteroscedasticity? Plot each against residuals
plot(cpmodel, resid(., scaled = T) ~ fitted(.))

# heteroscedasticity among different levels of the random effects?
plot(cpmodel, resid(., scaled = T) ~ fitted(.) | id)

# normal distribution of residuals?
hist(resid(cpmodel))
qqnorm(resid(cpmodel))
qqline(resid(cpmodel))

# normal distribution of random intercepts?
hist(ranef(cpmodel)[[1]])
qqnorm(ranef(cpmodel)[[1]])
qqline(ranef(cpmodel)[[1]])

# normal distribution od random slopes?
hist(ranef(cpmodel)[[2]])
qqnorm(ranef(cpmodel)[[2]])
qqline(ranef(cpmodel)[[2]])
...
ordprocess<-process[order(process$relqual.x, process$slope, process$id, process$conflict),]
